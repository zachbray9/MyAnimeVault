...
export default observer(function CarouselCard({ anime }: Props) {
    ...
    return (
        <Box as={NavLink} to={`/anime/${anime.id}/details`} key={anime.id} position='relative' gap={4} bg='surface.1' cursor='pointer' onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} >
            ...
            <Box position='absolute' top='-.5rem' bottom='-.5rem' right='-.5rem' left='-.5rem' opacity={isHovered ? 1 : 0} transition='opacity 0.2s ease'>
